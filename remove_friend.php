...
<?php
	...
	if (mysqli_num_rows($result) == 0) {
		...
		if (mysqli_num_rows($result) == 0) {
			print "<h4><span style=\"color:red\">Error</span>: Cannot remove nonexistant friendship.</h4>";
		} else {
			$query = "DELETE FROM friends WHERE userOne='$currentUser' AND userTwo='$userName'";
			$result = mysqli_query($db, $query)
				or die ("Error: Could not remove friend.");
			$query = "DELETE FROM friends WHERE userOne='$userName' AND userTwo='$currentUser'";
			$result = mysqli_query($db, $query)
				or die ("Error: Could not remove friend.");
			print "<p>Success! <span style=\"color:#ff6666\">$charName - $realm</span> removed from your friends.</p>";
		}
	}
?>
